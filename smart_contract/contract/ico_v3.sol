...
contract IcoContract is Supporter{
    ...
    BonusReferal public bonusReferal;
    ...
    function setBonusReferal(address _bonusReferal) onlyOwner public {
        bonusReferal = BonusReferal(_bonusReferal);
    } 
    ...
    function confirmListKycHasParent(address[] list, address[] listParent) onlySupporter public{
        for (uint i = 0; i < list.length; i++) {
            ...
            if(kyc[list[i]]!=true){
                kyc[list[i]] = true;
                bonusReferal.addParentReferal(list[i],listParent[i]);
            }
        }
    }
    ...
    function confirmKycHasParent(address add, address parent) onlySupporter public{
        ...
        if(kyc[add]!=true){
            kyc[add] = true;
            bonusReferal.addParentReferal(add,parent);
        }
    }
    ...
    function switchRound(bool _isLocked, uint _value, address _address) public returns(bool success){
        ...
        if (currentTime <= preIcoEndDate) {
            ...
            if (isLocked) {
                ...
                if(totalEthBuyTokenLockInPreIco[sender] >= 0.05 ether){
                    bonusLockPreIco[sender] = totalEthBuyTokenLockInPreIco[sender].mul(10).div(100).div(pricePreIcoLocked);
                }else{
                    if(totalEthBuyTokenLockInPreIco[sender] >= 0.01 ether){
            ...
            if (currentTime >= icoStartDate && currentTime <= icoEndDate) {
                ...
                if (total_invest_eth_ico <= maxInvestICO) {
                    ...
                    if (isLocked) {
                        ...
                        if(totalEthBuyTokenUnlockInIco[sender] >= 0.01 ether){
    ...
    function sendListAllBonus(address[] list, uint[] listBonus) public onlyOwner{
        ...
        uint referal = 0;
        for (uint i = 0; i < list.length; i++) {
            referal = bonusReferal.getReferal(list[i]);
            if (list[i] != 0x0 && (listBonus[i] > 0 || bonusIco[list[i]] > 0 || bonusPreIco[list[i]]>0 || referal > 0)) {
                uniBotToken.transferToken(list[i],listBonus[i].mul(10 ** 18).add(bonusIco[list[i]]).add(bonusPreIco[list[i]]).add(referal), false);
                bonusIco[list[i]] = 0;
                bonusPreIco[list[i]] = 0;
                bonusReferal.updateBonusReferal(list[i]);
            }
        }
    }
...
contract BonusReferal is Owned{
    using SafeMath for uint;
    UniBotToken public uniBotToken;
    ...
    uint public totalReferal;
    uint public totalAccountReferal;
    
    
    function BonusReferal(address _icoContract) public {
        icoContractAddress = _icoContract;
    }
    
    function setUniBotToken(address add) public {
        uniBotToken = UniBotToken(add);
    }
    
    function addParentReferal(address add, address parent){
        require(msg.sender == icoContractAddress);
        if(add != 0x0 && parent!=0x0 &&  referalParent[add] == 0x0){
            if(amountReferal[parent] == 0){
                totalAccountReferal = totalAccountReferal.add(2);
            }else{
                totalAccountReferal = totalAccountReferal.add(1);
            }
            referalParent[add] = parent;
            amountReferal[parent] = amountReferal[parent].add(1);
        }
    }
    
    function referalBefore(address sender) public {
        require(msg.sender == icoContractAddress);
        require(referalParent[sender]!=0x0);
        ...
        totalReferal = totalReferal.sub(referal_temp*2);
    }
    
    function referalAfter(address sender) public {
        require(msg.sender == icoContractAddress);
        require(referalParent[sender]!=0x0);
        ...
        totalReferal = totalReferal.sub(referal_temp_after*2);
    }
    
    function updateBonusReferal(address sender) public {
        require(msg.sender == icoContractAddress);
        totalReferal = totalReferal.sub(bonusReferal[sender]);
        bonusReferal[sender] = 0;
        totalAccountReferal = totalAccountReferal.sub(1);
    }
    
    function getReferal(address sender) public returns (uint){
        return bonusReferal[sender];
    }
}
